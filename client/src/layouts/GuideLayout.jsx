...
import { FaChartPie, FaMapMarkedAlt, FaUser, FaSignOutAlt, FaChevronDown } from "react-icons/fa";
...
const GuideLayout = () => {
  ...
  // Get User Data
  const [user, setUser] = useState(JSON.parse(sessionStorage.getItem("user")) || { fullName: "Guide", role: "Guide" });
  ...
  const NavItem = ({ to, icon: Icon, label }) => {
    ...
    return (
      <Link to={to} className={`relative flex items-center gap-4 px-6 py-3 text-sm font-medium transition-all duration-300 rounded-xl mx-2
        ${active 
          ? "bg-teal-800 text-white shadow-lg translate-x-1" 
  ...
  return (
    <div className="flex h-screen bg-[#F3F4F6] font-sans overflow-hidden">
      ...
      {/* SIDEBAR - Teal #0f766e */}
      <div className="w-64 bg-[#0f766e] flex flex-col border-r border-teal-900 shrink-0">
        <div className="p-8 pb-8">
          <h1 className="text-2xl font-bold text-white tracking-tight">Planora.</h1>
          <p className="text-teal-200 text-xs font-medium uppercase tracking-wide mt-1">Tour Guide</p>
        </div>

        <nav className="flex-1 overflow-y-auto no-scrollbar space-y-1 pb-4">
          <NavItem to="/guide/overview" icon={FaChartPie} label="Overview" />
          <NavItem to="/guide/bookings" icon={FaMapMarkedAlt} label="My Bookings" />
          <NavItem to="/guide/profile" icon={FaUser} label="My Profile" />
        </nav>
      </div>
          ...
          <div className="relative">
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-3 hover:bg-gray-50 p-2 rounded-lg transition-colors">
              <div className="text-right hidden md:block">
                <p className="text-sm font-bold text-teal-900">Guide</p>
                <p className="text-xs text-gray-500 font-medium">{user.fullName}</p>
              </div>
              <img src={user.profilePicture || "https://cdn-icons-png.flaticon.com/512/3135/3135715.png"} className="w-10 h-10 rounded-full object-cover border border-teal-200" />
