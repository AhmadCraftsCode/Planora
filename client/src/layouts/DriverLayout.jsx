...
import { FaChartPie, FaCar, FaUser, FaSignOutAlt, FaChevronDown, FaRoute } from "react-icons/fa";
...
const DriverLayout = () => {
  ...
  // Get User Data
  const [user, setUser] = useState(JSON.parse(sessionStorage.getItem("user")) || { fullName: "Driver", role: "Driver" });
  ...
  // Sidebar Link - Orange/Bronze Theme
  const NavItem = ({ to, icon: Icon, label }) => {
    ...
    return (
      <Link to={to} className={`relative flex items-center gap-4 px-6 py-3 text-sm font-medium transition-all duration-300 rounded-xl mx-2
        ${active 
          ? "bg-orange-700 text-white shadow-lg translate-x-1" 
  ...
  return (
    <div className="flex h-screen bg-[#F3F4F6] font-sans overflow-hidden">
      ...
      {/* SIDEBAR - Burnt Orange / Bronze #c2410c */}
      <div className="w-64 bg-[#c2410c] flex flex-col border-r border-orange-900 shrink-0">
        <div className="p-8 pb-8">
          <h1 className="text-2xl font-bold text-white tracking-tight">Planora.</h1>
          <p className="text-orange-200 text-xs font-medium uppercase tracking-wide mt-1">Driver Panel</p>
        </div>

        <nav className="flex-1 overflow-y-auto no-scrollbar space-y-1 pb-4">
          <NavItem to="/driver/overview" icon={FaChartPie} label="Overview" />
          <NavItem to="/driver/bookings" icon={FaRoute} label="My Bookings" />
          <NavItem to="/driver/profile" icon={FaUser} label="My Profile" />
        </nav>
      </div>
          ...
          <div className="relative">
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-3 hover:bg-gray-50 p-2 rounded-lg transition-colors">
              <div className="text-right hidden md:block">
                <p className="text-sm font-bold text-orange-900">Driver</p>
                <p className="text-xs text-gray-500 font-medium">{user.fullName}</p>
              </div>
              <img src={user.profilePicture || "https://cdn-icons-png.flaticon.com/512/3135/3135715.png"} className="w-10 h-10 rounded-full object-cover border border-orange-200" />
