...
const Home = () => {
  ...
  // Get User for Conditional Rendering
    ...
    <div className="min-h-screen font-sans bg-slate-50">
            ...
            <div className="bg-white rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center border border-gray-100">
              <div className="flex-1 w-full border-r border-gray-100 pr-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input 
                    type="text" 
                    placeholder={activeTab === 'drivers' ? "Pickup City" : "Destination"} 
              ...
              <div className="flex-1 w-full pr-4">
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1 ml-1">Max Budget (PKR)</label>
                <div className="flex items-center gap-2">
      ...
      <section id="packages" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Only show if user exists AND is a Customer */}
          ...
          {loading ? <p className="text-center mt-10">Loading packages...</p> : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
              {data.packages.length > 0 ? data.packages.map((pkg) => (
                ...
                />
              )) : <p className="text-center col-span-3 text-gray-400">No packages available.</p>}
            </div>
          )}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-10">
            {data.hotels.length > 0 ? data.hotels.map((hotel) => (
              ...
              />
            )) : <p className="text-center col-span-4 text-gray-400">No hotels registered yet.</p>}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
            {data.drivers.length > 0 ? data.drivers.map((driver) => (
              ...
              />
            )) : <p className="text-center col-span-3 text-gray-400">No drivers available yet.</p>}
          </div>
        </div>
      </section>
...
const DriverCard = ({ name, car, city, rate, image, onBook }) => (
  <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-xl border border-gray-200 hover:border-orange-200 hover:bg-orange-50 transition cursor-pointer group">
    <div className="w-16 h-16 rounded-full bg-gray-300 overflow-hidden border border-gray-200">
    ...
    <div className="flex-1">
      ...
      <p className="text-xs text-gray-500 flex items-center gap-1 mt-1"><FaCar /> {car}</p>
    </div>
