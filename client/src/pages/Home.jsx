...
const Home = () => {
  ...
  // Removed 'guides' from initial state if you aren't using them
  const [data, setData] = useState({ packages: [], hotels: [], drivers: [] }); 
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get("/api/public/search", {
        params: { type: activeTab, location: searchLocation, price: searchPrice }
      });
  ...
  return (
    <div className="min-h-screen font-sans bg-slate-50">
      ...
      {selectedItem && (
        <BookingModal item={selectedItem.item} type={selectedItem.type} onClose={() => setSelectedItem(null)} />
      )}

      {/* HERO SECTION */}
      <div className="relative h-[85vh] w-full bg-slate-900 flex items-center justify-center px-4">
        <div className="absolute inset-0 bg-cover bg-center opacity-60" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1589802829985-817e51171b92?q=80&w=2070&auto=format&fit=crop')" }}></div>
        ...
        <div className="relative z-10 text-center w-full max-w-4xl mt-10">
          ...
          <p className="text-gray-300 text-lg mb-10 max-w-2xl mx-auto">
            Plan your entire journey in one place. Book tours, luxury hotels, and reliable drivers seamlessly.
          </p>
          ...
          <div className="bg-white p-2 rounded-3xl shadow-2xl max-w-3xl mx-auto backdrop-blur-sm bg-white/90">
            <div className="flex gap-2 mb-2 p-1">
              {["packages", "hotels", "drivers"].map(tab => (
                <button
                  key={tab}
            ...
            <div className="bg-white rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center border border-gray-100">
              <div className="flex-1 w-full border-r border-gray-100 pr-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="text" placeholder={activeTab === 'drivers' ? "Pickup City" : "Destination"} className="w-full text-sm font-semibold text-slate-700 outline-none" value={searchLocation} onChange={(e) => setSearchLocation(e.target.value)} />
                </div>
              </div>
              <div className="flex-1 w-full pr-4">
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1 ml-1">
                   {activeTab === 'drivers' ? "Max Rate (PKR/km)" : "Max Budget (PKR)"}
                </label>
                <div className="flex items-center gap-2">
                  <FaMoneyBillWave className="text-green-500" />
                  <input type="number" placeholder="e.g. 50000" className="w-full text-sm font-semibold text-slate-700 outline-none" value={searchPrice} onChange={(e) => setSearchPrice(e.target.value)} />
                </div>
              </div>
              <button onClick={handleSearch} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-xl shadow-lg transition-all transform hover:scale-105">
                <FaSearch size={20} />
              </button>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {user && user.role === "Customer" && (
            <div className="text-center mt-6">
              <button onClick={() => navigate("/customer/create-custom")} className="bg-slate-800 text-white px-6 py-2.5 rounded-full text-sm font-bold hover:bg-slate-900 transition shadow-lg flex items-center gap-2 mx-auto">
          ...
          {loading ? <p className="text-center mt-10">Loading packages...</p> : (
            // Added 'snap-mandatory' and 'scroll-pl-4' for better spacing
            <div className="flex overflow-x-auto gap-6 mt-10 pb-8 px-4 no-scrollbar snap-x snap-mandatory">
              {data.packages.length > 0 ? data.packages.map((pkg) => (
                <div key={pkg._id} className="min-w-[320px] md:min-w-[350px] shrink-0 snap-start"> {/* Fixed Width + Shrink 0 */}
                  <PackageCard 
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex overflow-x-auto gap-6 mt-10 pb-8 px-4 no-scrollbar snap-x snap-mandatory">
            {data.hotels.length > 0 ? data.hotels.map((hotel) => (
              <div key={hotel._id} className="min-w-[300px] shrink-0 snap-start">
                <HotelCard 
...
// ... (Keep Sub Components SectionHeader, PackageCard, HotelCard, DriverCard exactly as they were) ...
// Copy them from the previous working code.
...
const PackageCard = ({ image, title, price, location, rating, duration, onBook }) => (
  <div className="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 h-full">
    <div className="h-56 overflow-hidden relative">
      ...
      <div className="absolute top-3 left-3 bg-black/60 backdrop-blur px-2 py-1 rounded text-xs font-bold text-white">{duration} Days</div>
      <div className="absolute top-3 right-3 bg-white/90 backdrop-blur px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1 text-orange-500"><FaStar /> {rating}</div>
    </div>
    <div className="p-6">
      <h3 className="text-lg font-bold text-slate-800 line-clamp-1">{title}</h3>
      <div className="flex items-center gap-2 text-sm text-gray-500 mt-2 mb-4"><FaMapMarkerAlt className="text-blue-500" /> {location}</div>
      <div className="flex justify-between items-center border-t border-gray-100 pt-4">
        <div><p className="text-xs text-gray-400 font-bold uppercase">Starting From</p><p className="text-lg font-bold text-blue-600">PKR {price}</p></div>
...
const HotelCard = ({ image, name, city, price, onBook }) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition h-full">
    <div className="h-40 overflow-hidden"><img src={image} alt={name} className="w-full h-full object-cover" /></div>
    <div className="p-4">
      ...
      <div className="mt-3 flex justify-between items-end">
         <div><p className="text-sm font-bold text-purple-600">PKR {price}</p><p className="text-[10px] text-gray-400">per night</p></div>
